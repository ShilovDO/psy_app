...
async def create_config(
    ...
    result = (
        ...
        .filter(Configs.id == new_config.id)
        .first()
    )  # Используем first(), так как ID уникален
...
async def all_config(
    ...
    if sort == 0 or sort == "0":
        ...
        service_configs = (
            db.query(Configs)
            .filter(Configs.owner == user_id, Configs.service == sort)
            .count()
        )
...
async def all_config_for_route(request: Request, db: Session = Depends(get_db)):
    ...
    configs = (
        db.query(Configs)
        .filter((Configs.owner == user_id) | (Configs.owner == None))
        .all()
    )
    return configs

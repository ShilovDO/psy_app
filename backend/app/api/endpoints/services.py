...
async def create_service(service: NewService, request: Request, db: Session = Depends(get_db)):
...
async def all_service(field, direction, page: int = 1, per_page: int = 10, db: Session = Depends(get_db)):
    # Вычисляем смещение
...
async def available_service(field, direction, page: int = 1, per_page: int = 10, db: Session = Depends(get_db)):
    # Вычисляем смещение
    offset = (page - 1) * per_page

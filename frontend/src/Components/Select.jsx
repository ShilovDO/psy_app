...
const Select = ({ 
    ...
    const filteredOptions = options.filter(opt => {
        if (opt[valueKey] === 0) return false;
        if (!search) return true;
        return opt[labelKey]?.toLowerCase().includes(search.toLowerCase());
    });
    ...
    return (
        <div className={`relative ${className}`} ref={dropdownRef}>
            ...
            >
                {selectedOption && selectedOption[valueKey] !== 0 ? (
                ...
                {clearable && selectedOption && selectedOption[valueKey] !== 0 && (
                    <button
                        onClick={handleClear}
                        ...
                        {filteredOptions.length === 0 ? (
                            ...
                            filteredOptions.map((option) => (
                                <div
                                    ...
                                    className={`p-2.5 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors ${
                                        option[valueKey] === value ? 'bg-gray-100 dark:bg-gray-600' : ''
                                    }`}
                                >
                                    <div className="flex items-center gap-3">
                                        <span className="flex-1 text-left text-gray-900 dark:text-white break-words">
                                            {option[labelKey]}
                                        </span>
